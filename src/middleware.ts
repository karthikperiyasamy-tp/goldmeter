...
export function middleware(request: NextRequest) {
  ...
  // Includes: Googlebot, Bingbot, Google's Rich Results Test, Lighthouse, SEO tools, etc.
  const ua = request.headers.get("user-agent") || "";
  const isBot = /bot|crawl|spider|slurp|bingpreview|lighthouse|pagespeed|google|facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegram|sitechecker|ahrefs|semrush|moz|screaming|seobility/i.test(ua);
  if (isBot) {
